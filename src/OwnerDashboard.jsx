          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))', gap:12 }}>
            ...
            <div><label style={lbl}>Plan</label>
              <select value={newForm.plan} onChange={e=>setNewForm({...newForm,plan:e.target.value})} style={inp}>
                <option value="trial">Trial — {planLimits.trial} tables max (7 days)</option>
                <option value="starter">Starter — {planLimits.starter} tables max ({currency.position==='before'?`${currency.symbol} ${planPrices.starter}`:`${planPrices.starter} ${currency.symbol}`}/mo)</option>
                <option value="restaurant">Restaurant — {planLimits.restaurant} tables max ({currency.position==='before'?`${currency.symbol} ${planPrices.restaurant}`:`${planPrices.restaurant} ${currency.symbol}`}/mo)</option>
                <option value="enterprise">Enterprise — {planLimits.enterprise>=999?'unlimited':planLimits.enterprise+' max'} ({currency.position==='before'?`${currency.symbol} ${planPrices.enterprise}`:`${planPrices.enterprise} ${currency.symbol}`}/mo)</option>
              </select>
            </div>
          </div>
