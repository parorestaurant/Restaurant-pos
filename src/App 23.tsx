...
import { loadKey, saveKey, checkConnection, getCurrentMode, setLocalServerIP, getLocalServerIP } from './lib/storage';
...
function getTableStatus(n, tickets, bills, calls, reservations=[]) {
  ...
  const now = Date.now();
  const bookingPending = calls.some((c) => {
    if (c.table !== n || c.type !== 'booking') return false;
    if (c.status !== 'pending' && c.status !== 'seen') return false;
    try { return new Date(c.date + 'T' + (c.time||'23:59')).getTime() > now - 3600000; } catch { return true; }
  });
  if (callPending) return { key: 'calling', label: 'Calling staff', color: COLORS.urgent, pulse: true };
  if (bookingPending) return { key: 'booked', label: 'Booked', color: COLORS.well, pulse: false };
...
function TicketCard({ ticket, now }) {
        ...
        {ticket.items.map((it, i) => (
          <li key={i} style={{ fontSize: 14, padding: '5px 0', borderTop: i === 0 ? 'none' : `1px solid ${COLORS.ink}15` }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 6 }}>
              <div style={{ flex: 1, textDecoration: it.ready ? 'line-through' : 'none', opacity: it.ready ? 0.5 : 1 }}>
              ...
              {it.ready && <span style={{ fontSize: 11, color: COLORS.good, fontWeight: 700, flexShrink: 0 }}>✓ Ready</span>}
            </div>
          </li>
        ))}
      </ul>
      <div style={{ marginTop: 12, width: '100%', textAlign: 'center', padding: '8px 0', fontFamily: "'Inter', sans-serif", fontWeight: 600, fontSize: 13, letterSpacing: 0.3, color: accent, border: `1px solid ${accent}44`, borderRadius: 3, background: `${accent}11` }}>
        {ticket.status === 'new' ? '📋 Received' : ticket.status === 'preparing' ? '👨‍🍳 Preparing' : '✅ Ready'}
      </div>
    </div>
  );
}

function StationBoard({ station, tickets, now }) {
          ...
          return (
            <div key={lane.key}>
              ...
              {laneTickets.length === 0 ? (
                ...
                laneTickets.map((t) => <TicketCard key={t.id} ticket={t} now={now} />)
              )}
            </div>
          );
        })}
...
function KoreanRestaurantPOS() {
  ...
  const [serverMode, setServerMode] = useState('cloud');
  const [localIP, setLocalIP] = useState(() => { try { return getLocalServerIP(); } catch { return ''; } });
  ...
  const [tabFontSize, setTabFontSize] = useState(13);
  const [tabPadding, setTabPadding] = useState(7);
  const [tabActiveColor, setTabActiveColor] = useState('#D9622B');
  const [tabBg, setTabBg] = useState('transparent');
  ...
  useEffect(() => {
    ...
    (async () => {
      const [ok, m, t, b, c, rv, cfg] = await Promise.all([
        checkConnection(),
      ...
      setConnected(ok); setServerMode(getCurrentMode());
      ...
      if (cfg.tabFontSize) setTabFontSize(cfg.tabFontSize);
      if (cfg.tabPadding) setTabPadding(cfg.tabPadding);
      if (cfg.tabActiveColor) setTabActiveColor(cfg.tabActiveColor);
      if (cfg.tabBg) setTabBg(cfg.tabBg);
  ...
  const pendingBookings = []; // Booking feature disabled — entrance QR now shows table availability only
  ...
  function acknowledgeCall(id) {
    const updated = calls.map((c) => {
      if (c.id !== id) return c;
      // For bookings: mark as 'seen' so they stay in history until booking time passes
      // For calls: mark as 'done' to remove
      const newStatus = c.type === 'booking' ? 'seen' : 'done';
  ...
  async function saveConfig(patch) {
    const cfg = { ownerPin, staffPin, kitchenPin, barPin, tableCount, bankQR, pageBg, headerBg, fontFamily, fontSize, fontColor, textBg, headingBg, headingSize, defaultServiceCharge, defaultGst, restaurantName, menuCategories, currencySymbol, currencyPosition, currencyDecimals, tabFontSize, tabPadding, tabActiveColor, tabBg, ...patch };
    ...
    if (patch.tabFontSize !== undefined) setTabFontSize(patch.tabFontSize);
    if (patch.tabPadding !== undefined) setTabPadding(patch.tabPadding);
    if (patch.tabActiveColor !== undefined) setTabActiveColor(patch.tabActiveColor);
    if (patch.tabBg !== undefined) setTabBg(patch.tabBg);
  ...
  return (
    <div style={{ minHeight: '100vh', background: pageBg ? `url(${pageBg}) center/cover fixed` : COLORS.bg, fontFamily: `'${fontFamily}', sans-serif`, fontSize: fontSizePx, color: fontColor }}>
      ...
      <div style={{ borderBottom: `1px solid ${COLORS.border}`, background: headerBg ? `url(${headerBg}) center/cover` : COLORS.panel }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <span>
            ...
            <span style={{ fontSize: 10, color: connected ? (serverMode==='local' ? COLORS.warn : COLORS.good) : COLORS.urgent, fontFamily: "'JetBrains Mono', monospace", marginLeft: 10 }}>{connected ? (serverMode==='local' ? '● local server' : '● synced') : '○ offline — check backend'}</span>
          </span>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div style={{ position: 'relative' }}>
                  <button onClick={() => setShowCalls((s) => !s)} style={{ ...iconBtnStyle, width: 34, height: 34, position: 'relative', border: `1px solid ${pendingCalls.length ? COLORS.urgent : COLORS.border}` }}>
                    ...
                    {(pendingCalls.length + pendingBookings.length) > 0 && (
                      <span style={{ position: 'absolute', top: -4, right: -4, background: pendingBookings.length > 0 ? COLORS.well : COLORS.urgent, color: '#fff', borderRadius: 8, fontSize: 10, minWidth: 16, height: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'JetBrains Mono', monospace" }}>{pendingCalls.length + pendingBookings.length}</span>
                    )}
                  </button>
                  {showCalls && (
                    <div style={{ position: 'absolute', right: 0, top: '110%', background: COLORS.panel, border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: 10, width: 260, zIndex: 70 }}>
                      ...
                      {pendingCalls.length === 0 && pendingBookings.length === 0 ? (
                        ...
                        <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 4px', borderBottom: `1px solid ${COLORS.border}55` }}>
                          <span style={{ fontSize: 12, color: COLORS.cream }}>{c.message || `Table ${c.table} calling`}</span>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 20px 60px', background: textBg, minHeight: 'calc(100vh - 62px)' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 20 }}>
              {categories.map((cat, ci) => {
                // Auto-select first category if none selected (replaces old "All" tab)
                const isActive = selectedCategory === cat || (!selectedCategory && ci === 0);
                return (
                  <button key={cat} onClick={() => { setSelectedCategory(cat); setSelectedDrinkSub(null); }} style={{ padding: `${tabPadding}px ${tabPadding*2}px`, borderRadius: 20, fontSize: tabFontSize, fontWeight: 600, cursor: 'pointer', border: `1px solid ${isActive ? tabActiveColor : COLORS.border}`, background: isActive ? `${tabActiveColor}22` : tabBg, color: isActive ? tabActiveColor : COLORS.muted }}>{cat}</button>
                );
              })}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3" style={{ gap: 28 }}>
              <div className="md:col-span-2">
                {(selectedCategory ? [selectedCategory] : categories.slice(0,1)).map((cat) => {
        ...
        {view === 'kitchen' && !isGuestMode && <StationBoard station="kitchen" tickets={tickets} now={now} />}
        {view === 'bar' && !isGuestMode && <StationBoard station="bar" tickets={tickets} now={now} />}
        ...
        {view === 'floor' && !isGuestMode && (
          ...
          {/* Today's bookings summary */}
            {(() => {
              const todayBookings = (calls||[]).filter(c => c.type==='booking' && (c.status==='pending' || c.status==='seen'));
                ...
                <div style={{ background: COLORS.panel, border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: 18, maxWidth: 480 }}>
                  ...
                  {tableTickets.length > 0 && (
                    <div style={{ marginBottom: 12 }}>
                      {tableTickets.map((t) => (
                        <div key={t.id} style={{ fontSize: 12, color: COLORS.muted, padding: '4px 0', borderBottom: `1px solid ${COLORS.border}33` }}>
                          <div style={{ fontWeight: 700, marginBottom: 2 }}>{STATION_META[t.station].label} — <span style={{ color: t.status === 'ready' ? COLORS.warn : COLORS.ember }}>{t.status}</span></div>
                          {t.items.map((i, idx) => (
                            <div key={idx} style={{
                              textDecoration: i.ready ? 'line-through' : 'none',
                              opacity: i.ready ? 0.5 : 1,
                              color: i.ready ? COLORS.good : COLORS.cream,
                              fontSize: 12, padding: '1px 0'
                            }}>
                              {i.qty}× {i.name} {i.ready && '✓'}
                            </div>
                          ))}
                        </div>
                      ))}
        ...
        {view === 'settings' && authRole === 'owner' && (
          <div style={{ maxWidth: 480 }}>
            ...
            <div style={{ background: COLORS.panel, border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: 18, marginBottom: 16 }}>
              <h3 style={{ ...dynSubTitleStyle, marginTop: 0 }}>🔌 Local Server (Offline Mode)</h3>
              <p style={{ fontSize: 12, color: COLORS.muted, marginBottom: 12 }}>If you have a mini PC running TableFire locally inside your restaurant, enter its IP address here. The app will automatically switch to it whenever the internet goes down — and switch back when internet returns.</p>
              <input
                value={localIP}
                onChange={e => setLocalIP(e.target.value)}
                onBlur={e => { const v = e.target.value.trim(); setLocalServerIP(v); }}
                placeholder="e.g. 192.168.1.100:8787"
                style={inputStyle}
              />
              <div style={{ marginTop: 10, padding: '10px 14px', background: serverMode === 'local' ? `${COLORS.good}18` : `${COLORS.ember}11`, border: `1px solid ${serverMode === 'local' ? COLORS.good + '44' : COLORS.ember + '33'}`, borderRadius: 6, fontSize: 12, color: serverMode === 'local' ? COLORS.good : COLORS.muted }}>
                {serverMode === 'local' ? '✅ Currently connected to local server — working offline!' : '☁️ Currently using cloud server'}
              </div>
              <p style={{ fontSize: 11, color: COLORS.muted, marginTop: 8 }}>Leave blank if you don't have local mode set up — TableFire will just use the cloud server.</p>
            </div>

            <div style={{ background: COLORS.panel, border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: 18, marginBottom: 16 }}>
              <h3 style={{ ...dynSubTitleStyle, marginTop: 0 }}>Menu category tabs</h3>
              <p style={{ fontSize: 12, color: COLORS.muted, marginBottom: 12 }}>Customize the look of category tabs (Starters, Mains, Drinks etc.) on the table ordering screen.</p>

              <label style={labelStyle}>Tab font size</label>
              <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
                {[11, 13, 15, 17].map(s => (
                  <button key={s} onClick={() => { setTabFontSize(s); saveConfig({ tabFontSize: s }); }} style={{ flex: 1, padding: '7px 0', borderRadius: 4, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: `1px solid ${tabFontSize === s ? COLORS.ember : COLORS.border}`, background: tabFontSize === s ? `${COLORS.ember}22` : 'transparent', color: tabFontSize === s ? COLORS.ember : COLORS.cream }}>{s}px</button>
                ))}
              </div>

              <label style={labelStyle}>Tab size (padding)</label>
              <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
                {[{label:'Small',v:5},{label:'Medium',v:7},{label:'Large',v:10}].map(p => (
                  <button key={p.v} onClick={() => { setTabPadding(p.v); saveConfig({ tabPadding: p.v }); }} style={{ flex: 1, padding: '7px 0', borderRadius: 4, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: `1px solid ${tabPadding === p.v ? COLORS.ember : COLORS.border}`, background: tabPadding === p.v ? `${COLORS.ember}22` : 'transparent', color: tabPadding === p.v ? COLORS.ember : COLORS.cream }}>{p.label}</button>
                ))}
              </div>

              <label style={labelStyle}>Active tab color</label>
              <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
                {['#D9622B', '#5C9D6E', '#4A90D9', '#D9A12B', '#C73E3E', '#A855F7'].map(c => (
                  <button key={c} onClick={() => { setTabActiveColor(c); saveConfig({ tabActiveColor: c }); }} style={{ width: 36, height: 36, borderRadius: '50%', border: tabActiveColor === c ? `3px solid ${COLORS.cream}` : '1px solid #0003', background: c, cursor: 'pointer' }} />
                ))}
              </div>

              <label style={labelStyle}>Tab background (inactive tabs)</label>
              <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
                {[{label:'Transparent',v:'transparent'},{label:'Panel',v:COLORS.panel},{label:'Dark',v:'#00000033'}].map(b => (
                  <button key={b.v} onClick={() => { setTabBg(b.v); saveConfig({ tabBg: b.v }); }} style={{ flex: 1, padding: '7px 0', borderRadius: 4, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: `1px solid ${tabBg === b.v ? COLORS.ember : COLORS.border}`, background: tabBg === b.v ? `${COLORS.ember}22` : 'transparent', color: tabBg === b.v ? COLORS.ember : COLORS.cream }}>{b.label}</button>
                ))}
              </div>

              {/* Live preview */}
              <div style={{ marginTop: 10, padding: '12px', background: COLORS.bg, borderRadius: 6, border: `1px solid ${COLORS.border}` }}>
                <div style={{ fontSize: 11, color: COLORS.muted, marginBottom: 8 }}>Preview:</div>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  <button style={{ padding: `${tabPadding}px ${tabPadding*2}px`, borderRadius: 20, fontSize: tabFontSize, fontWeight: 600, border: `1px solid ${tabActiveColor}`, background: `${tabActiveColor}22`, color: tabActiveColor }}>Starters</button>
                  <button style={{ padding: `${tabPadding}px ${tabPadding*2}px`, borderRadius: 20, fontSize: tabFontSize, fontWeight: 600, border: `1px solid ${COLORS.border}`, background: tabBg, color: COLORS.muted }}>Mains</button>
                  <button style={{ padding: `${tabPadding}px ${tabPadding*2}px`, borderRadius: 20, fontSize: tabFontSize, fontWeight: 600, border: `1px solid ${COLORS.border}`, background: tabBg, color: COLORS.muted }}>Drinks</button>
                </div>
              </div>
            </div>
...
export default KoreanRestaurantPOS;
