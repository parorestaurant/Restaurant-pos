...
import StationApp from './StationApp.jsx';
...
function Root() {
  ...
  // Component is now resolved instantly via static imports — no dynamic import() delay,
  // which was the cause of the blank screen on first load (needed a refresh to show up).
  useEffect(() => {
    if (isStation) { setComp(() => StationApp); return; }
    if (isDemo) { setComp(() => DemoApp); return; }
    if (isOwner) { setComp(() => OwnerDashboard); return; }
    if (isCustomer) { setComp(() => CustomerApp); return; }
