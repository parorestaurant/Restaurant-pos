...
function KoreanRestaurantPOS() {
  ...
  const pendingBookings = calls.filter((c) => c.type === 'booking' && (c.status === 'pending' || c.status === 'seen')).filter(c => {
    // Keep booking visible until its booking time has passed (date + time)
    try {
      const bookingDateTime = new Date(c.date + 'T' + (c.time || '23:59')).getTime();
      return bookingDateTime > Date.now() - 3600000; // keep 1 hour after booking time
    } catch { return true; }
