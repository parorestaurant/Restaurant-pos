...
function PayBillView({ table, code }) {
  ...
  React.useEffect(() => {
    async function load() {
      try {
        const res = await fetch(`${API}/state/${encodeURIComponent(code+':bills')}`);
        const data = await res.json();
  ...
  const total = bill ? bill.lines.reduce((a,l) => a + (l.price * l.qty), 0) : 0;
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', padding:'14px 0', fontWeight:800, fontSize:18 }}>
            <span>Total</span>
            <span style={{ color:C.ember }}>{money(total)}</span>
          </div>
          {!requested ? (
