  ...
  async function callStaff() {
    ...
    try {
      await fetch(`${API}/state/${encodeURIComponent(code+':calls')}`, {
        ...
        body: JSON.stringify({ value: [{ table: Number(table), type:'call', status:'pending', calledAt: Date.now() }] })
      });
      setCalled(true);
    } catch(e) {}
    setLoading(false);
    ...
    <div style={{ padding:'20px' }}>
      ...
      {loading ? (
        ...
        <>
          ...
          {!requested ? (
            <button onClick={()=>setRequested(true)} style={{ width:'100%', padding:16, background:C.ember, border:'none', color:'#fff', borderRadius:10, cursor:'pointer', fontFamily:'inherit', fontSize:16, fontWeight:800, marginTop:8 }}>
