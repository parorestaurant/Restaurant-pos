...
import { UtensilsCrossed, CalendarDays, MapPin, Plus, Minus, ShoppingCart, X, QrCode, Bell, CreditCard } from 'lucide-react';
...
function MenuView({ cart, onAddToCart, onRemoveFromCart }) {
  ...
  return (
    <div>
      {/* Category tabs — matches table tablet style exactly, no "All" tab */}
      <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginBottom:20 }}>
        {categories.map(cat=>(
          <button key={cat} onClick={()=>setSelectedCat(cat)} style={{ padding:'7px 14px', borderRadius:20, fontSize:13, fontWeight:600, cursor:'pointer', border:`1px solid ${activeCat===cat?C.ember:C.border}`, background:activeCat===cat?`${C.ember}22`:'transparent', color:activeCat===cat?C.ember:C.muted }}>{cat}</button>
        ))}
      </div>

      {/* Category heading — matches table tablet's subTitleStyle */}
      <h3 style={{ fontFamily:"'Oswald',sans-serif", textTransform:'uppercase', letterSpacing:1.5, fontSize:15, color:C.cream, borderBottom:`1px solid ${C.border}`, paddingBottom:6, marginBottom:14 }}>{activeCat}</h3>

      {/* Grid of menu cards — matches table tablet's MenuCard grid layout */}
      <div style={{ display:'grid', gridTemplateColumns:'repeat(2, 1fr)', gap:12 }}>
        {filtered.map(item=>(
          <div key={item.id} style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:8, overflow:'hidden', display:'flex', flexDirection:'column' }}>
            <div style={{ width:'100%', aspectRatio:'4/3', overflow:'hidden', background:'#2A2620', display:'flex', alignItems:'center', justifyContent:'center' }}>
              {item.image ? <img src={item.image} alt={item.name} style={{ width:'100%', height:'100%', objectFit:'cover' }}/> : <UtensilsCrossed size={28} color={C.muted}/>}
            </div>
            <div style={{ padding:'10px 12px', flex:1, display:'flex', flexDirection:'column' }}>
              <div style={{ color:C.cream, fontSize:13, fontWeight:600, lineHeight:1.3, minHeight:34 }}>{item.name}</div>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:8 }}>
                <div style={{ color:C.ember, fontSize:15, fontWeight:700 }}>{money(item.price)}</div>
                <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                  {cartQty(item.id)>0 && <>
                    <button onClick={()=>onRemoveFromCart(item.id)} style={{ width:26,height:26,borderRadius:6,border:`1px solid ${C.border}`,background:'transparent',color:C.cream,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center' }}><Minus size={12}/></button>
                    <span style={{ color:C.cream, fontSize:13, minWidth:14, textAlign:'center' }}>{cartQty(item.id)}</span>
                  </>}
                  <button onClick={()=>onAddToCart(item)} style={{ width:26,height:26,borderRadius:6,border:'none',background:C.ember,color:'#fff',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center' }}><Plus size={12}/></button>
                </div>
              </div>
            </div>
          </div>
...
// Order view — always shows the LIVE persisted order history for this table,
// fetched from the server, so it survives navigating away and coming back
// (just like the table tablet's "View bill" — no more order history disappearing)
function OrderView({ cart, onClearCart, onRemoveItem }) {
  ...
  // Load live order history for this table — refreshes every few seconds
  // so customers always see their current order status, even after leaving
  // and coming back to this tab.
  useEffect(()=>{
    if (!tableNum) return;
    let cancelled = false;
    async function loadHistory() {
      try {
        const [billRes, ticketsRes] = await Promise.all([
          fetch(`${API}/state/${encodeURIComponent(_code+':bills')}`),
          fetch(`${API}/state/${encodeURIComponent(_code+':tickets')}`),
        ]);
        const billData = await billRes.json();
        const ticketsData = await ticketsRes.json();
        if (cancelled) return;
        const bills = billData.value || [];
        const tickets = ticketsData.value || [];
        const bill = bills.find(b => b.table === Number(tableNum) && b.status === 'open' && b.lines?.length > 0);
        const myT = tickets.filter(t => t.table === Number(tableNum) && t.status !== 'cancelled');
        setMyBill(bill || null);
        setMyTickets(myT);
      } catch(e) {}
    }
    loadHistory();
    const interval = setInterval(loadHistory, 6000);
  ...
  async function placeOrder(cidDetails={}) {
    ...
    try {
      ...
      const result = await api('/customer/orders', { method:'POST', body:{ table:parseInt(tableNum), items, code:_code, customerName:cidDetails.name||'Table '+tableNum, customerCID:cidDetails.cid||'', customerPhone:cidDetails.phone||'' }});
      // Immediately reflect the new order — the polling loop above will keep it fresh after this
      if (result.bill) setMyBill(result.bill);
      if (result.tickets) setMyTickets(prev => [...prev, ...result.tickets]);
      onClearCart();
    } catch(e) { setFlash(e.message); }
    setPlacing(false);
  }

  const subtotal = myBill?.lines?.reduce((s,l)=>s+l.price*l.qty,0) || 0;
    ...
    <div>
      ...
      {cart.length===0 ? (
        ...
        <>
          ...
          <button onClick={()=>{ if(!tableNum){setFlash('Please enter your table number');return;} setFlash(''); if(_table){ placeOrder(); } else { setShowCID(true); } }} disabled={placing} style={btn(tableNum?C.ember:'#555')}>
      ...
      {myTickets.length > 0 && (
        <div style={{ marginTop:24 }}>
          <div style={{ fontSize:12, color:C.muted, textTransform:'uppercase', letterSpacing:1, marginBottom:12 }}>Your order history — Table {tableNum}</div>
          {myTickets.map(t=>(
...
function BookingHistoryView() {
  ...
  React.useEffect(() => {
    async function load() {
      try {
        const res = await fetch(`${API}/state/${encodeURIComponent(_code+':bookings')}`);
        const data = await res.json();
        const all = data.value || [];
        // Show recent bookings
        setBookings(all.sort((a,b) => b.createdAt - a.createdAt).slice(0, 10));
      } catch(e) {}
      setLoading(false);
    }
    load();
  ...
  const statusColor = { confirmed:C.good, pending:C.warn, cancelled:C.urgent };
  const statusLabel = { confirmed:'✅ Confirmed', pending:'⏳ Pending', cancelled:'❌ Cancelled' };

  if (loading) return <div style={{ textAlign:'center', padding:40, color:C.muted }}>Loading bookings...</div>;

  if (bookings.length === 0) return (
    <div style={{ textAlign:'center', padding:40 }}>
      <div style={{ fontSize:40, marginBottom:12 }}>📅</div>
      <div style={{ color:C.muted }}>No bookings found yet</div>
    </div>
  );

  return (
    <div style={{ padding:'0 16px' }}>
      <div style={{ fontWeight:700, fontSize:16, color:C.cream, marginBottom:16 }}>Recent Bookings</div>
      {bookings.map((b, i) => (
        <div key={i} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:10, padding:'14px 16px', marginBottom:10 }}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
            <div style={{ fontWeight:700, color:C.cream }}>{b.customerName}</div>
            <span style={{ fontSize:12, color: statusColor[b.status]||C.muted, fontWeight:700 }}>
              {statusLabel[b.status]||b.status}
            </span>
          </div>
          <div style={{ fontSize:13, color:C.muted }}>📅 {b.date} · ⏰ {b.time}</div>
          <div style={{ fontSize:13, color:C.muted }}>👥 {b.partySize} people{b.table ? ` · Table ${b.table}` : ''}</div>
          {b.notes && <div style={{ fontSize:12, color:C.muted, marginTop:4, fontStyle:'italic' }}>{b.notes}</div>}
        </div>
      ))}
    </div>
  );
...
// ── Call Staff View ───────────────────────────────────────
function CallStaffView({ table, code }) {
  ...
  async function callStaff() {
    setLoading(true);
    try {
      const res = await fetch(`${API}/state/${encodeURIComponent(code+':calls')}`);
      const data = await res.json();
      ...
      const newCalls = [...calls, {
        ...
        table: Number(table),
        type: 'call',
        message: `🔔 Table ${table} is calling staff`,
        status: 'pending',
        calledAt: Date.now(),
        createdAt: Date.now(),
      }];
      await fetch(`${API}/state/${encodeURIComponent(code+':calls')}`, {
        method:'PUT',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ value: newCalls })
      });
      setCalled(true);
    } catch(e) {}
    setLoading(false);
  ...
  return (
    <div style={{ padding:'40px 20px', textAlign:'center' }}>
      ...
      {called ? (
        <>
          <div style={{ fontWeight:800, fontSize:24, color:C.good, marginBottom:10 }}>Staff is coming!</div>
          <div style={{ color:C.muted, fontSize:14 }}>Someone will be at Table {table} shortly.</div>
          <button onClick={()=>setCalled(false)} style={{ marginTop:24, padding:'12px 28px', background:C.card, border:`1px solid ${C.border}`, color:C.muted, borderRadius:8, cursor:'pointer', fontFamily:'inherit', fontSize:14 }}>Call again</button>
        </>
      ) : (
        <>
          <div style={{ fontWeight:800, fontSize:22, color:C.cream, marginBottom:10 }}>Need assistance?</div>
          <div style={{ color:C.muted, fontSize:14, marginBottom:28 }}>Table {table} · Staff will come to you</div>
          <button onClick={callStaff} disabled={loading} style={{ padding:'16px 40px', background:C.ember, border:'none', color:'#fff', borderRadius:10, cursor:'pointer', fontFamily:'inherit', fontSize:16, fontWeight:800, width:'100%' }}>
            {loading ? 'Calling...' : '🔔 Call Staff'}
          </button>
        </>
      )}
    </div>
...
// ── Pay Bill View ─────────────────────────────────────────
function PayBillView({ table, code }) {
  const [bill, setBill] = React.useState(null);
  const [loading, setLoading] = React.useState(true);
  const [requested, setRequested] = React.useState(false);
  const [cfg, setCfg] = React.useState({ defaultGst:0, defaultServiceCharge:0, currencySymbol:'Nu' });

  React.useEffect(() => {
    async function load() {
      try {
        const [billRes, cfgRes] = await Promise.all([
          fetch(`${API}/state/${encodeURIComponent(code+':bills')}`),
          fetch(`${API}/state/${encodeURIComponent(code+':config')}`)
        ]);
        const billData = await billRes.json();
        const cfgData = await cfgRes.json();
        const bills = billData.value || [];
        const myBill = bills.find(b => b.table === Number(table) && b.status === 'open');
        setBill(myBill || null);
        if (cfgData.value) setCfg(cfgData.value);
      } catch(e) {}
      setLoading(false);
    }
    load();
  ...
  const subtotal = bill ? bill.lines.reduce((a,l) => a + (l.price * l.qty), 0) : 0;
  const gstRate = cfg.defaultGst || 0;
  const serviceRate = cfg.defaultServiceCharge || 0;
  const gstAmt = Math.round(subtotal * gstRate / 100);
  const serviceAmt = Math.round(subtotal * serviceRate / 100);
  const total = subtotal + gstAmt + serviceAmt;
  const sym = cfg.currencySymbol || 'Nu';

  return (
    <div style={{ padding:'20px' }}>
      <div style={{ fontWeight:800, fontSize:20, color:C.cream, marginBottom:16 }}>💳 Your Bill — Table {table}</div>
      {loading ? (
        <div style={{ color:C.muted, textAlign:'center', padding:40 }}>Loading bill...</div>
      ) : !bill ? (
        <div style={{ textAlign:'center', padding:40 }}>
          <div style={{ fontSize:40, marginBottom:12 }}>✅</div>
          <div style={{ color:C.muted }}>No open bill found for Table {table}</div>
        </div>
      ) : (
        <>
          {bill.lines.map((line, i) => (
            <div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'10px 0', borderBottom:`1px solid ${C.border}`, fontSize:14 }}>
              ...
              <span style={{ color:C.amber }}>{money(line.price * line.qty)}</span>
            </div>
          ))}
          <div style={{ display:'flex', justifyContent:'space-between', padding:'8px 0', fontSize:14, color:C.muted, borderTop:`1px solid ${C.border}`, marginTop:4 }}>
            <span>Subtotal</span>
            <span>{sym} {subtotal}</span>
          </div>
          {gstRate > 0 && <div style={{ display:'flex', justifyContent:'space-between', padding:'6px 0', fontSize:13, color:C.muted }}>
            <span>GST ({gstRate}%)</span>
            <span>{sym} {gstAmt}</span>
          </div>}
          {serviceRate > 0 && <div style={{ display:'flex', justifyContent:'space-between', padding:'6px 0', fontSize:13, color:C.muted }}>
            <span>Service Charge ({serviceRate}%)</span>
            <span>{sym} {serviceAmt}</span>
          </div>}
          <div style={{ display:'flex', justifyContent:'space-between', padding:'12px 0', fontWeight:800, fontSize:18, borderTop:`1px solid ${C.border}`, marginTop:4 }}>
            <span>Total</span>
            <span style={{ color:C.ember }}>{sym} {total}</span>
          </div>
          {!requested ? (
            <button onClick={async ()=>{
              ...
              try {
                ...
                const newCalls = [...calls, {
                  ...
                  type: 'pay-bill',
                  table: Number(table),
                  message: `💳 Bill requested — Table ${table} (${sym} ${total})`,
                  status: 'pending',
                  createdAt: Date.now(),
                }];
                await fetch(`${API}/state/${encodeURIComponent(code+':calls')}`, {
            ...
            }} style={{ width:'100%', padding:16, background:C.ember, border:'none', color:'#fff', borderRadius:10, cursor:'pointer', fontFamily:'inherit', fontSize:16, fontWeight:800, marginTop:8 }}>
              Request Bill at Table
            </button>
          ) : (
            <div style={{ textAlign:'center', padding:20, background:`${C.good}18`, border:`1px solid ${C.good}44`, borderRadius:10, marginTop:8 }}>
              <div style={{ fontWeight:700, color:C.good }}>✅ Staff will bring your bill!</div>
              <div style={{ fontSize:12, color:C.muted, marginTop:6 }}>Someone will be at Table {table} shortly.</div>
            </div>
          )}
        </>
      )}
...
export default function CustomerApp() {
  ...
  const tabs = _table ? [
    ...
    { key:'call',   label:'Call Staff', Icon:Bell },
    { key:'pay',    label:'Pay Bill',   Icon:CreditCard },
  ] : [
  ...
  return (
    <div style={{ minHeight:'100vh', background:C.bg, fontFamily:'Inter,sans-serif', color:C.cream }}>
      ...
      <div style={{ maxWidth:480, margin:'0 auto', padding:'20px 16px', paddingBottom:90 }}>
        ...
        {view==='call'   && <CallStaffView table={_table} code={_code}/>}
        {view==='pay'    && <PayBillView table={_table} code={_code}/>}
        ...
        {view==='history' && <BookingHistoryView/>}
      </div>
