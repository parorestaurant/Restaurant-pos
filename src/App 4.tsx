...
import { loadKey, saveKey, checkConnection, onModeChange, setLocalServerIP } from './lib/storage';
...
function KoreanRestaurantPOS() {
  ...
  useEffect(() => {
    onModeChange((mode) => setServerMode(mode));
  }, []); // 'cloud' | 'local'
  const [localIP, setLocalIP] = useState(() => { try { return localStorage.getItem('tf_local_server_ip') || ''; } catch { return ''; } });
      ...
      <div style={{ borderBottom: `1px solid ${COLORS.border}`, background: headerBg ? `url(${headerBg}) center/cover` : COLORS.panel }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <span>
            ...
            <span style={{ fontSize: 10, color: connected ? (serverMode==='local' ? '#F59E0B' : COLORS.good) : COLORS.urgent, fontFamily: "'JetBrains Mono', monospace", marginLeft: 10 }}>{connected ? (serverMode==='local' ? '● local server' : '● cloud synced') : '○ offline'}</span>
          </span>
        ...
        {view === 'settings' && authRole === 'owner' && (
          <div style={{ maxWidth: 480 }}>
            ...
            <div style={{ background: COLORS.panel, border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: 18, marginBottom: 16 }}>
              ...
              <p style={{ fontSize: 12, color: COLORS.muted, marginBottom: 12 }}>Set your mini PC local IP so TableFire works even without internet. Leave blank to use cloud only.</p>
              <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                <input
                  value={localIP}
                  onChange={e => setLocalIP(e.target.value)}
                  onBlur={e => { setLocalServerIP(e.target.value.trim()); }}
                  placeholder="e.g. 192.168.1.100:8787"
                  style={inputStyle}
                />
              </div>
              <div style={{ marginTop:10, padding:"8px 12px", background:serverMode==="local" ? "#5C9D6E22" : "#D9622B11", border:`1px solid ${serverMode==="local" ? "#5C9D6E44" : "#D9622B33"}`, borderRadius:6, fontSize:12, color:serverMode==="local" ? "#5C9D6E" : COLORS.muted }}>
                {serverMode==="local" ? "✅ Connected to local server — working offline!" : "☁️ Using cloud server — local server not detected"}
              </div>
            </div>
            <div style={{ background: COLORS.panel, border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: 18, marginBottom: 16 }}>
