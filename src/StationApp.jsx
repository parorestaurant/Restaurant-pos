...
export default function StationApp() {
  ...
  async function handleAdvance(ticketId, action, itemIdx) {
    // OPTIMISTIC UPDATE: update UI instantly using current state, sync to server in background
    if (action === 'ready-item') {
      const ticket = tickets.find(t => t.id === ticketId);
      const item = ticket?.items?.[itemIdx];
      const itemPatch = { items: ticket.items.map((it, i) => i === itemIdx ? { ...it, ready: true } : it) };
      const updated = tickets.map(t => t.id === ticketId ? { ...t, ...itemPatch } : t);
      setTickets(updated); // instant UI update
      // Protect this change from being overwritten by an in-flight poll
      pendingChangesRef.current[ticketId] = { patch: itemPatch, timestamp: Date.now() };
      isSavingRef.current = true;
      saveKey('tickets', updated).finally(() => { isSavingRef.current = false; });
      if (item && ticket) {
    ...
    setTickets(updated); // instant UI update
    // Protect this change from being overwritten by an in-flight poll
    const statusPatch = action === 'cancel-item' ? null : { status: updated.find(t=>t.id===ticketId)?.status };
    if (statusPatch) pendingChangesRef.current[ticketId] = { patch: statusPatch, timestamp: Date.now() };
    isSavingRef.current = true;
    saveKey('tickets', updated).finally(() => { isSavingRef.current = false; }); // background sync, no await blocking UI
