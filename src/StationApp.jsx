...
// Atomic update for a single ticket — avoids the race condition where
// Kitchen and Bar both rewrite the FULL tickets array at the same time
async function atomicUpdateTicket(code, ticketId, patch) {
  try {
    const res = await fetch(`${API_BASE}/tickets/${encodeURIComponent(code)}/${encodeURIComponent(ticketId)}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ patch }),
    });
    if (!res.ok) throw new Error('atomic update failed');
    return await res.json();
  } catch (e) {
    console.error('atomicUpdateTicket failed', e.message);
    return null;
  }
}

// Atomic update for a single item within a ticket — for the Ready button
async function atomicUpdateItem(code, ticketId, itemIdx, patch) {
  try {
    const res = await fetch(`${API_BASE}/tickets/${encodeURIComponent(code)}/${encodeURIComponent(ticketId)}/items/${itemIdx}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ patch }),
    });
    if (!res.ok) throw new Error('atomic item update failed');
    return await res.json();
  } catch (e) {
    console.error('atomicUpdateItem failed', e.message);
    return null;
  }
}
          ...
          <li key={i} style={{ padding:'5px 0', borderTop:i===0?'none':'1px solid #2A262015', fontSize:14 }}>
            {confirmCancel===i ? (
              ...
              <div style={{ display:'flex', alignItems:'flex-start', gap:6 }}>
                <div style={{ flex:1, textDecoration: it.ready ? 'line-through' : 'none', opacity: it.ready ? 0.5 : 1 }}><span style={{ fontWeight:700 }}>{it.qty}×</span> {it.name}
                  {it.notes&&<div style={{ fontSize:11, fontStyle:'italic', opacity:0.6, marginLeft:22 }}>{it.notes}</div>}
                </div>
                {!it.ready && <button onClick={()=>onAdvance(ticket.id,'ready-item',i)} style={{ height:20, borderRadius:3, border:'1px solid #5C9D6E55', background:'#5C9D6E18', color:'#5C9D6E', cursor:'pointer', padding:'0 6px', fontSize:11, fontWeight:700 }}>✓ Ready</button>}
                {it.ready && <span style={{ fontSize:11, color:'#5C9D6E', fontWeight:700 }}>✓ Served</span>}
...
export default function StationApp() {
  ...
  const [authed, setAuthed] = useState(() => {
    try { return localStorage.getItem('tf_station_authed_' + station) === 'true'; } catch { return false; }
  });
  ...
  const pendingChangesRef = useRef({}); // ticketId -> { items, status, timestamp } - protects optimistic updates from being overwritten by stale polls
  ...
  async function loadData() {
    try {
      const cfg = await loadKey('config', {});
      if (cfg) {
    ...
    try {
      // Don't poll while a save is in-flight — avoids overwriting our own optimistic update with stale data
      if (isSavingRef.current) return;

      const t = await loadKey('tickets', []);
      let active = (t||[]).filter(tk => tk.station === station && tk.status !== 'cancelled');
      const served = (t||[]).filter(tk => tk.station === station && tk.status === 'served');

      // Re-apply any pending optimistic changes that are newer than what server returned
      // (protects against a slightly-stale poll response undoing a button press)
      const now = Date.now();
      Object.entries(pendingChangesRef.current).forEach(([ticketId, pending]) => {
        if (now - pending.timestamp > 8000) {
          delete pendingChangesRef.current[ticketId]; // expire old pending entries
          return;
        }
        active = active.map(tk => tk.id === ticketId ? { ...tk, ...pending.patch } : tk);
      });
  ...
  async function handleAdvance(ticketId, action, itemIdx) {
    // OPTIMISTIC UPDATE: update UI instantly, sync to server using ATOMIC per-ticket endpoint
    // (NOT the full-array save — that was causing Kitchen and Bar to overwrite each other's tickets!)
    if (action === 'ready-item') {
      const ticket = tickets.find(t => t.id === ticketId);
      const item = ticket?.items?.[itemIdx];
      const newItems = ticket.items.map((it, i) => i === itemIdx ? { ...it, ready: true } : it);
      const updated = tickets.map(t => t.id === ticketId ? { ...t, items: newItems } : t);
      setTickets(updated); // instant UI update
      pendingChangesRef.current[ticketId] = { patch: { items: newItems }, timestamp: Date.now() };
      isSavingRef.current = true;
      atomicUpdateItem(restaurantCode, ticketId, itemIdx, { ready: true }).finally(() => { isSavingRef.current = false; });
      if (item && ticket) {
        const stationLabel = station === 'kitchen' ? '👨‍🍳 Kitchen' : '🍹 Bar';
        loadKey('calls', []).then(calls => {
          const newCalls = [...calls, {
            id: 'ready-' + Date.now(),
            type: 'item-ready',
            table: ticket.table,
            message: `${stationLabel}: ${item.qty}× ${item.name} ready — Table ${ticket.table}`,
            status: 'pending',
            createdAt: Date.now(),
          }];
          saveKey('calls', newCalls);
        });
      }
      return;
    }
    ...
    if (action === 'cancel-item') {
      ...
      const finalTicket = updated.find(t => t.id === ticketId);
      patch = finalTicket?.status === 'cancelled' ? { status: 'cancelled' } : { items: finalTicket.items };
    } else {
      updated = tickets.map(t => {
        if (t.id !== ticketId) return t;
        const next = t.status === 'new' ? 'preparing' : t.status === 'preparing' ? 'ready' : 'served';
        if (next === 'ready') becameReady = t;
        return { ...t, status: next };
      });
      patch = { status: updated.find(t=>t.id===ticketId)?.status };
    }
    setTickets(updated); // instant UI update
    pendingChangesRef.current[ticketId] = { patch, timestamp: Date.now() };
    isSavingRef.current = true;
    atomicUpdateTicket(restaurantCode, ticketId, patch).finally(() => { isSavingRef.current = false; });
    // Notify staff when WHOLE order is marked ready
    if (becameReady) {
      const stationLabel = station === 'kitchen' ? '👨‍🍳 Kitchen' : '🍹 Bar';
      const itemSummary = becameReady.items.map(i => `${i.qty}× ${i.name}`).join(', ');
      loadKey('calls', []).then(calls => {
        const newCalls = [...calls, {
          id: 'order-ready-' + Date.now(),
          type: 'item-ready',
          table: becameReady.table,
          message: `${stationLabel}: Whole order ready — Table ${becameReady.table} (${itemSummary})`,
          status: 'pending',
          createdAt: Date.now(),
        }];
        saveKey('calls', newCalls);
  ...
  if (!authed) return <PinScreen station={station} pin={pin} onSuccess={() => { setAuthed(true); try { localStorage.setItem('tf_station_authed_' + station, 'true'); } catch{} }}/>;
    ...
    <div style={{ minHeight:'100vh', background:COLORS.bg, fontFamily:'Inter,sans-serif', color:COLORS.cream }}>
      ...
      <div style={{ background:COLORS.panel, borderBottom:`1px solid ${COLORS.border}`, padding:'12px 20px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <button onClick={() => { setAuthed(false); try { localStorage.removeItem('tf_station_authed_' + station); } catch{} }} style={{ padding:'6px 12px', background:'transparent', border:`1px solid ${COLORS.border}`, borderRadius:6, color:COLORS.muted, fontSize:12, cursor:'pointer' }}>Lock</button>
        </div>
      </div>
