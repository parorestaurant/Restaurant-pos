...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info) { 
    console.error('TableFire crash:', error.message, error.stack); 
    try { localStorage.setItem('tf_last_error', error.message + '\n' + error.stack); } catch{} 
  }
  render() {
    if (this.state.crashed) return (
      <div style={{ minHeight:'100vh', background:C.bg, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:16, padding:24, fontFamily:'Inter,sans-serif' }}>
        ...
        <div style={{ fontSize:11, color:'#C73E3E', maxWidth:340, textAlign:'center', fontFamily:'monospace', wordBreak:'break-word' }}>{(() => { try { return localStorage.getItem('tf_last_error')?.slice(0,200) } catch{} })()}</div>
