...
// ── Per-key write lock to prevent race conditions ──────────
// When two requests read-modify-write the same key simultaneously,
// the second write can overwrite the first (lost update problem).
// This lock serializes writes to the same key so they queue up safely.
...
// Atomic read-modify-write: locks the key, reads fresh value, applies updater, saves
async function dbUpdate(key, updater) {
...
// ── Atomic single-ticket update ────────────────────────────
// Prevents the race condition where Kitchen and Bar tablets both
// read-modify-write the FULL tickets array at the same time, causing
// one device's changes to silently overwrite the other's (tickets disappearing).
// This endpoint locks the key, updates just ONE ticket by id, and saves —
// so concurrent updates from different stations never clobber each other.
app.put('/tickets/:code/:ticketId', async (req, res) => {
  const { code, ticketId } = req.params;
  const patch = req.body.patch || {};
  const tKey = `${code}:tickets`;
  const updated = await dbUpdate(tKey, (current) => {
    const all = current || [];
    return all.map(t => t.id === ticketId ? { ...t, ...patch } : t);
  });
  const updatedTicket = updated.find(t => t.id === ticketId);
  res.json({ ok: true, ticket: updatedTicket });
});

// ── Atomic single-item-within-ticket update (for Ready button) ──
app.put('/tickets/:code/:ticketId/items/:itemIdx', async (req, res) => {
  const { code, ticketId, itemIdx } = req.params;
  const patch = req.body.patch || {};
  ...
  const updated = await dbUpdate(tKey, (current) => {
    ...
    return all.map(t => {
      ...
      return { ...t, items: t.items.map((it, i) => i === idx ? { ...it, ...patch } : it) };
    });
  });
  const updatedTicket = updated.find(t => t.id === ticketId);
  res.json({ ok: true, ticket: updatedTicket });
...
app.post('/customer/orders', async (req, res) => {
  ...
  // CID only required for walk-in orders without table number
  if (!customerName) return res.status(400).json({ error: 'Name required' });
  if (!table && !customerCID) return res.status(400).json({ error: 'CID required for walk-in orders' });

  const mKey = code ? `${code}:menu` : 'menu';
  const tKey = code ? `${code}:tickets` : 'tickets';
  const bKey = code ? `${code}:bills` : 'bills';
  const menu = await dbGet(mKey);
  ...
  // Use atomic update lock to prevent race condition when multiple customers order simultaneously
  await dbUpdate(tKey, (current) => [...(current||[]), ...newTickets]);

  let resultBill;
  await dbUpdate(bKey, (current) => {
    const allBills = current || [];
  ...
  res.json({ tickets:newTickets, bill:resultBill, billId:resultBill.id });
});
