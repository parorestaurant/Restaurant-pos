...
// ── Supabase storage (persists across Render restarts) ────
const SUPABASE_URL    = process.env.SUPABASE_URL    || 'https://bqkxzgrwqmmchjiecsud.supabase.co';
const SUPABASE_SECRET = process.env.SUPABASE_SECRET || 'sb_secret_Au95sbiG0w0LGn4ey6R9oA_ezevDpq2';
...
async function dbSet(key, value) {
  try {
    ...
    if (!res.ok) {
      ...
      console.error('dbSet error', key, err);
    }
  } catch(e) {
...
async function dbDelete(key) {
  try {
    ...
    console.error('dbDelete error', key, e.message);
  }
}
...
const OWNER_EMAIL   = process.env.OWNER_EMAIL   || 'tpee00767@gmail.com';
const OWNER_PASS    = process.env.OWNER_PASS    || 'tablefire2026';
...
app.post('/platform/tenants', authOwner, async (req, res) => {
  ...
  const expiresAt = Date.now() + (plan === 'trial' ? 7 : 30)*24*60*60*1000;
